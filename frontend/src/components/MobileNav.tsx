  ...
  return (
    <aside className="fixed right-0  lg:hidden w-full sm:w-[50%] h-[100vh]  bg-white z-30 shadow p-4 transition-transform translate-0   starting:translate-x-[100vh] linear duration-400  
      ...
      <div className="flex justify-between items-center">
        ...
        <X className="text-gray-500 text-base cursor-pointer" onClick={closeMobileNav} />
      </div>
      <hr className="my-3" />
      {!isAuthenticated ? (<button onClick={async () => await loginWithRedirect()} className="bg-orange-400 p-2 text-center text-white w-full rounded-lg cursor-pointer">Log In</button>) : <MobileNavLinks />}
