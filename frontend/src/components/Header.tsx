  ...
  return (
    <header className="border-b-2 border-orange-500  h-18 absolute bg-white top-0 w-full z-10">
      <nav className={`${paddingX} flex h-full justify-between items-center`}>
        <Link to="/">
          {" "}
          <h1 className="text-2xl md:text-3xl font-semibold text-orange-600">
            FD Foody
          </h1>
        </Link>
        <div className="hidden md:block">
          {isAuthenticated ? (
            ...
            <button
              className=" text-orange-600 font-semibold p-2 cursor-pointer"
              onClick={loginClick}
            >
              Log In
            </button>
        ...
        {!showMobileNav && (
          ...
          >
            {" "}
            <List className="text-orange-500 cursor-pointer" />
          </button>
        )}
      </nav>
    </header>
